...
- `loadEnv` refuses a `.env` symlinked outside the project. A monorepo or dotfiles layout pointing `.env` at a shared file outside the tree stops resolving; a symlink within the project still works. `api_call`, `run_with_env` and `get_env_keys` all surface the refusal as a warning rather than letting it read as an absent `.env`.
