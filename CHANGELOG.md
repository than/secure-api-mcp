...
- `sync_env_example` regenerates a stored placeholder byte-identical to the live value when it has a placeholder of its own to substitute. `SLACK_WEBHOOK=https://hooks.slack.com/services/...` carries its secret in the path, clearing the key regex, the scanner and the URL-credential checks.
