...
- `sync_env_example` regenerates a stored placeholder byte-identical to the live value unless the value is short and word-shaped. An empty generated placeholder is `smartPlaceholder`'s strongest refusal, not an absence of opinion, so keying on it permitted reuse exactly where the value had been judged unsafe to echo — `MAILGUN_SENDING=8f3a9c2e1b7d40561122` names no known brand and matched no gate.
...
- `loadEnv` refuses a `.env` symlinked outside the project. A monorepo or dotfiles layout pointing `.env` at a shared file outside the tree stops resolving; a symlink within the project still works. `api_call` surfaces the refusal as a warning rather than letting it read as an absent `.env`.
